...
start_if_missing surreal "export PATH=/home/sunshuheng/.surrealdb:\$PATH; '$SURREAL_BIN' start --log info --user '$DB_USER' --pass '$DB_PASS' --bind '$DB_HOST:$DB_PORT' rocksdb:'$DB_PATH'"
...
start_if_missing frontend "cd '$FRONTEND_DIR' && export PORT='$FRONTEND_PORT' && if command -v npm >/dev/null 2>&1; then npm run dev -- --hostname '$FRONTEND_HOST' --port '$FRONTEND_PORT'; else /home/sunshuheng/.nvm/versions/node/v22.19.0/bin/npm run dev -- --hostname '$FRONTEND_HOST' --port '$FRONTEND_PORT'; fi"
