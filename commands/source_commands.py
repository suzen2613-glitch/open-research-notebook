...
from open_notebook.domain.notebook import Asset, Source, SourceWikiCard
    ...
    try:
        ...
        try:
            ...
            if source:
                ...
                source.asset = source.asset or Asset(
                    file_path=input_data.content_state.get("file_path"),
                    url=input_data.content_state.get("url"),
                )
                await source.save()
            ...
            logger.warning(
                f"Failed to persist source error state for {input_data.source_id}: {save_error}"
            )
        return SourceProcessingOutput(
            success=False,
...
async def generate_wiki_card_command(
    ...
    try:
        ...
        wiki_card.research_context = payload["research_context"]
        wiki_card.claimed_gap = payload["claimed_gap"]
        wiki_card.positioning_summary = payload["positioning_summary"]
        ...
        wiki_card.embedding = payload.get("embedding")
