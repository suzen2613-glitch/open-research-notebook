...
from api.models import (
    ...
    WikiCardSearchRequest,
    WikiCardSearchResponse,
...
@router.post("/wiki-cards/search", response_model=WikiCardSearchResponse)
async def search_wiki_cards(request: WikiCardSearchRequest) -> WikiCardSearchResponse:
    """Vector search over wiki cards using card_embedding."""
    ...
    try:
        query_vectors = await embedding_model.aembed([query])
    except Exception as exc:
        logger.error(f"Failed to embed wiki card search query: {exc}")
        raise HTTPException(status_code=502, detail=f"Failed to embed query: {exc}")

    if not query_vectors:
        raise HTTPException(status_code=502, detail="Empty embedding returned for query")
    query_vector = query_vectors[0]

    params: Dict[str, Any] = {
        "query": query_vector,
        "min_similarity": request.minimum_score,
        "limit": request.limit,
    }
    notebook_filter = ""
    if request.notebook_id:
        params["notebook_id"] = normalize_record_id_string(request.notebook_id)
        notebook_filter = "AND $notebook_id IN notebook_ids"
    ...
    try:
        ...
        logger.error(f"Wiki card vector search failed: {exc}")
    ...
    for row in rows or []:
        results.append(
            WikiCardSearchResult(
                id=normalize_record_id_string(str(row.get("id") or "")),
                source_id=normalize_record_id_string(str(row.get("source_id") or "")) or None,
                title=row.get("title"),
                ...
                is_key_paper=bool(row.get("is_key_paper") or False),
                similarity=float(row.get("similarity") or 0.0),
            )
        )

    return WikiCardSearchResponse(results=results, total_count=len(results))
