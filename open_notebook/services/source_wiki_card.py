...
def build_card_embedding_text(card: Dict[str, Any]) -> str:
    """Compose a high-signal text representation of a wiki card for embedding.

    Uses normalized fields only (no raw source text) so the embedding captures
    the structured knowledge rather than noisy prose. Empty fields are skipped
    so short cards don't emit headers with blank values.
    """
    lines: List[str] = []
    for label, field_name in CARD_EMBEDDING_FIELD_ORDER:
...
async def generate_card_embedding(card: Dict[str, Any]) -> Optional[List[float]]:
    """Generate an embedding vector for a wiki card.

    Returns None if no embedding model is configured or the card has no usable
    text. Never raises — embedding is an opt-in capability and should not block
    card generation.
    """
    ...
    if not await model_manager.get_embedding_model():
        logger.debug("No embedding model configured; skipping wiki card embedding")
        return None
    ...
    try:
        ...
        logger.warning(f"Failed to embed wiki card: {exc}")
        return None
...
- domains 只能从以下值中选择一个或多个：electromagnetics, fluid_dynamics, solid_mechanics, heat_transfer, wave_propagation, multi_physics, inverse_problem, scientific_ml
...
- research_context: 基于 introduction / opening sections，总结研究背景、问题场景和论文所处脉络，80-240 字
- claimed_gap: 作者明确声称现有工作哪里不足、遗漏或不适用，40-160 字；原文未明确说明则返回空字符串
- positioning_summary: 用 1-3 句话概括这篇论文如何定位自己、为什么值得读，50-180 字
...
- recommended_entry_points: 适合导航入口的 canonical concept 或 domain 标识，例如 concept:pinn、domain:electromagnetics
...
SOURCE_WIKI_CARD_INTRO_PROMPT = """
你正在为 Open Notebook 的 Wiki Card 提取论文 introduction / related work 中的研究定位信息。

目标：
- 只根据提供的 INTRODUCTION_SECTION、RELATED_WORK_SECTION、CANONICAL_SUMMARY 生成结果
- 输出必须是单个合法 JSON 对象
- 不要输出 Markdown、解释、前后缀说明或代码围栏
- 若原文未明确说明，返回空字符串，不要推断
...
DOMAIN_ALIASES = {
    ...
    "multiphysics": "multi_physics",
    "multi_physics": "multi_physics",
    "inverse problem": "inverse_problem",
    "inverse problems": "inverse_problem",
    "inverse_problem": "inverse_problem",
    "scientific ml": "scientific_ml",
    "scientific machine learning": "scientific_ml",
    "scientific_ml": "scientific_ml",
}
...
DOMAIN_DISPLAY_LABELS = {
    ...
    "inverse_problem": "Inverse Problems",
...
MOC_GROUP_HUB_LABELS = {
    "concept:pinn": "PINN",
    "concept:deeponet": "DeepONet",
    "concept:neural-operator": "Neural Operators",
    "concept:fourier-neural-operator": "Neural Operators",
    "concept:operator-learning": "Operator Learning",
}
...
INTRO_CONTEXT_FIELDS = (
    "research_context",
    "claimed_gap",
    "positioning_summary",
)
INTRO_SECTION_ALIASES = {
    "introduction",
    "intro",
    "opening",
}
RELATED_WORK_SECTION_ALIASES = {
    "related work",
    "related works",
    "related_work",
    "related-work",
...
SECTION_NUMBERING_PREFIX_RE = re.compile(
    r"^(?:chapter\s+\d+|section\s+\d+|[ivxlcdm]+|[a-z]|\d+(?:\.\d+)*)(?:[\.\)\-:]|\s)+",
    re.IGNORECASE,
)
MAX_CONTEXT_SECTION_CHARS = 5000
OPENING_FALLBACK_CHARS = 4000
INTRO_POSITIONING_KEYWORDS = (
    "in this work",
    "in this paper",
    "we propose",
    "we present",
...
MAX_INTRO_RESEARCH_CONTEXT_SENTENCES = 3
MAX_INTRO_GAP_SENTENCES = 2
MAX_INTRO_POSITIONING_SENTENCES = 2
...
CANONICAL_CONCEPT_ALIASES: Dict[str, tuple[str, str]] = {
    ...
    **_register_concept_aliases(
        "concept:fourier-neural-operator",
...
def _normalize_section_heading(line: str) -> Optional[str]:
    stripped = line.strip()
    if not stripped:
        return None
    ...
    candidate = SECTION_NUMBERING_PREFIX_RE.sub('', candidate)
    candidate = candidate.strip(' :-–—\t')
    ...
    if len(candidate) <= 80 and (
        normalized in INTRO_SECTION_ALIASES
        or normalized in RELATED_WORK_SECTION_ALIASES
        or normalized in ABSTRACT_SECTION_ALIASES
    ):
        return normalized
...
def _extract_markdown_section(full_text: str, aliases: set[str]) -> Optional[str]:
    ...
    for position, (start_idx, heading) in enumerate(headings):
        ...
        if not section:
            return None
        return section[:MAX_CONTEXT_SECTION_CHARS].strip()

    return None


def _extract_opening_body_excerpt(full_text: str) -> str:
    text = full_text.strip()
    if not text:
        return ""

    abstract_section = _extract_markdown_section(text, ABSTRACT_SECTION_ALIASES)
    if abstract_section:
        start = text.find(abstract_section) + len(abstract_section)
        remainder = text[start:].strip()
        if remainder:
            return remainder[:OPENING_FALLBACK_CHARS].strip()

    return text[:OPENING_FALLBACK_CHARS].strip()


def extract_context_sections(full_text: str) -> Dict[str, str]:
    introduction = _extract_markdown_section(full_text, INTRO_SECTION_ALIASES)
    related_work = _extract_markdown_section(full_text, RELATED_WORK_SECTION_ALIASES)

    if not introduction:
        introduction = _extract_opening_body_excerpt(full_text)
...
def _normalize_intro_context_payload(payload: Dict[str, Any]) -> Dict[str, Optional[str]]:
    return {
        field: _normalize_optional_string(payload.get(field))
        for field in INTRO_CONTEXT_FIELDS
    }


def _clean_intro_section_text(text: str) -> str:
...
def _split_intro_sentences(text: str) -> List[str]:
    cleaned = _clean_intro_section_text(text)
    if not cleaned:
        return []
    ...
    seen: set[str] = set()
    for part in parts:
        sentence = _normalize_optional_string(part)
...
def _trim_intro_field(text: Optional[str], *, max_chars: int) -> Optional[str]:
    normalized = _normalize_optional_string(text)
    if not normalized:
        return None
...
def _extract_intro_context_fallback(
    *,
    context_sections: Dict[str, str],
    summary_block: str,
) -> Dict[str, Optional[str]]:
    introduction_sentences = _split_intro_sentences(context_sections.get('introduction') or '')
    related_sentences = _split_intro_sentences(context_sections.get('related_work') or '')
    combined_sentences = introduction_sentences + [
        sentence for sentence in related_sentences if sentence not in introduction_sentences
    ]

    research_context = None
    if introduction_sentences:
        research_context = " ".join(
            introduction_sentences[:MAX_INTRO_RESEARCH_CONTEXT_SENTENCES]
        )
    elif combined_sentences:
        research_context = " ".join(
            combined_sentences[:MAX_INTRO_RESEARCH_CONTEXT_SENTENCES]
        )
    else:
        research_context = _normalize_optional_string(summary_block)

    gap_candidates = [
        sentence
        for sentence in combined_sentences
        if any(keyword in sentence.casefold() for keyword in INTRO_GAP_KEYWORDS)
    ]
    claimed_gap = None
    if gap_candidates:
        claimed_gap = " ".join(gap_candidates[:MAX_INTRO_GAP_SENTENCES])
    elif len(introduction_sentences) > 1:
        claimed_gap = introduction_sentences[1]

    positioning_candidates = [
        sentence
        for sentence in introduction_sentences
        if any(keyword in sentence.casefold() for keyword in INTRO_POSITIONING_KEYWORDS)
    ]
    positioning_summary = None
    if positioning_candidates:
        positioning_summary = " ".join(
            positioning_candidates[:MAX_INTRO_POSITIONING_SENTENCES]
        )
    elif summary_block:
        summary_sentences = _split_intro_sentences(summary_block)
        if summary_sentences:
            positioning_summary = summary_sentences[0]
        else:
            positioning_summary = _normalize_optional_string(summary_block)

    return {
        'research_context': _trim_intro_field(research_context, max_chars=500),
        'claimed_gap': _trim_intro_field(claimed_gap, max_chars=320),
        'positioning_summary': _trim_intro_field(positioning_summary, max_chars=360),
    }


def _fill_missing_intro_context_fields(
    intro_payload: Dict[str, Optional[str]],
    fallback_payload: Dict[str, Optional[str]],
) -> Dict[str, Optional[str]]:
    merged = dict(intro_payload)
    for field in INTRO_CONTEXT_FIELDS:
...
def _merge_intro_context_fields(
    wiki_card_payload: Dict[str, Any],
    intro_payload: Dict[str, Optional[str]],
) -> Dict[str, Any]:
    updated = False
    for field in INTRO_CONTEXT_FIELDS:
        value = intro_payload.get(field)
        if value:
            wiki_card_payload[field] = value
            updated = True

    if updated:
        wiki_card_payload.update(_render_obsidian_exports(wiki_card_payload))
    return wiki_card_payload
...
def _build_moc_groups(
    *,
    ...
    if hub_label != "Scientific ML":
        groups.append(f"{hub_label} - Scientific ML")
    if not groups:
...
def _validate_intro_context_payload_schema(payload: Dict[str, Any]) -> None:
    for field_name in INTRO_CONTEXT_FIELDS:
        field_value = payload.get(field_name)
        if field_value is None:
            payload[field_name] = ""
            continue
        if not isinstance(field_value, str):
            raise ValueError(
                f"Intro context field '{field_name}' must be a string, got {type(field_value).__name__}"
            )
...
def _extract_intro_context_object(raw_text: str) -> Dict[str, Any]:
    return _extract_json_object(raw_text, validator=_validate_intro_context_payload_schema)
...
def render_obsidian_frontmatter(card: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "research_context": card.get("research_context"),
        "claimed_gap": card.get("claimed_gap"),
        "positioning_summary": card.get("positioning_summary"),
...
def render_obsidian_markdown(card: Dict[str, Any]) -> str:
    ...
    body: List[str] = [
        ...
        "",
        "## Research Context",
        card.get("research_context") or "Not stated in the source.",
        "",
        "## Claimed Gap",
        card.get("claimed_gap") or "Not stated in the source.",
        "",
        "## Positioning",
        card.get("positioning_summary") or "Not stated in the source.",
        "",
    ]
...
def normalize_wiki_card_payload(
    *,
    ...
    research_context = _normalize_optional_string(payload.get("research_context"))
    claimed_gap = _normalize_optional_string(payload.get("claimed_gap"))
    positioning_summary = _normalize_optional_string(payload.get("positioning_summary"))
...
async def generate_intro_context_payload(
    *,
    source: Source,
    summary_block: str,
    context_sections: Dict[str, str],
    ...
    introduction_section = (context_sections.get("introduction") or "").strip()
    related_work_section = (context_sections.get("related_work") or "").strip()
    if not introduction_section and not related_work_section:
        return {field: None for field in INTRO_CONTEXT_FIELDS}

    payload_messages = [
        SystemMessage(content=SOURCE_WIKI_CARD_INTRO_PROMPT),
        HumanMessage(
            content="\n\n".join(
                [
                    ...
                    f"CANONICAL_SUMMARY:\n{summary_block}",
                    f"INTRODUCTION_SECTION:\n{introduction_section or '(not found)'}",
                    f"RELATED_WORK_SECTION:\n{related_work_section or '(not found)'}",
                ]
            )
        ),
    ]
    ...
    parsed = _extract_intro_context_object(response_content)
    return _normalize_intro_context_payload(parsed)
...
async def generate_wiki_card_payload(
    ...
    context_sections = extract_context_sections(source.full_text)
    introduction_block = context_sections.get("introduction") or "(not found)"
    related_work_block = context_sections.get("related_work") or "(not found)"
    prompt_snapshot = (
        SOURCE_WIKI_CARD_PROMPT
        + "\n\n[INTRO_CONTEXT_PROMPT]\n"
        + SOURCE_WIKI_CARD_INTRO_PROMPT
    )
    ...
    response = await chain.ainvoke(payload_messages)
    response_content = extract_text_content(response.content)
    parsed = _extract_json_object(response_content, validator=_validate_wiki_card_payload_schema)
    ...
    if context_sections.get("introduction") or context_sections.get("related_work"):
        fallback_intro_payload = _extract_intro_context_fallback(
            context_sections=context_sections,
            summary_block=summary_block,
        )
        try:
            intro_payload = await generate_intro_context_payload(
                source=source,
                summary_block=summary_block,
                context_sections=context_sections,
                model_id=model_id,
            )
            intro_payload = _fill_missing_intro_context_fields(
                intro_payload,
                fallback_intro_payload,
            )
        except Exception as exc:
            logger.warning(
                f"Failed to enrich wiki card introduction context for {source.id}: {exc}"
            )
            intro_payload = fallback_intro_payload
        normalized = _merge_intro_context_fields(normalized, intro_payload)

    canonicalized = await canonicalize_wiki_card_record(normalized, source=source)
    canonicalized["embedding"] = await generate_card_embedding(canonicalized)
...
async def serialize_source_wiki_card(
    ...
    *,
    ...
    serialized = {
        ...
        "research_context": wiki_card.research_context,
        "claimed_gap": wiki_card.claimed_gap,
        "positioning_summary": wiki_card.positioning_summary,
        "topics": topics,
        "methods": methods,
        "problems": problems,
