...
_NUMBERED_SECTION_PREFIX_RE = re.compile(r"^\d+(?:\.\d+)+\.?\s+")
_NUMBERED_SIMPLE_PREFIX_RE = re.compile(r"^\d+\s+")
_ROMAN_SECTION_PREFIX_RE = re.compile(r"^[IVXLC]+[.)]\s+", re.IGNORECASE)
...
def _strip_section_numbering_prefix(line: str) -> str:
    stripped = line.strip()
    for pattern in (
        _NUMBERED_SECTION_PREFIX_RE,
        _ROMAN_SECTION_PREFIX_RE,
        _NUMBERED_SIMPLE_PREFIX_RE,
    ):
...
def _looks_like_numbered_section_heading(line: str) -> bool:
    stripped = line.strip()
    remainder = _strip_section_numbering_prefix(stripped)
    if remainder == stripped:
        return False

    if _SECTION_HEADING_RE.match(remainder):
        return True

    if _NUMBERED_SECTION_PREFIX_RE.match(stripped) or _ROMAN_SECTION_PREFIX_RE.match(
        stripped
    ):
        return True
...
def _is_probable_title_line(line: str, *, is_heading: bool = False) -> bool:
    stripped = line.strip().strip('"').strip("'").strip("*").strip("`")
    if not stripped:
        return False
    if len(stripped) < 12 or len(stripped) > 300:
        return False
    if _looks_like_numbered_section_heading(stripped):
        return False
    if _SECTION_HEADING_RE.match(stripped):
        return False
    if _SECTION_HEADING_RE.match(_strip_section_numbering_prefix(stripped)):
        return False
    ...
    if stripped.startswith("[") and stripped.endswith("]"):
        return False
    if not is_heading and _AUTHOR_LINE_RE.match(stripped):
        return False
    if stripped.count(" ") < 2:
        return False
...
def extract_paper_title_from_markdown(markdown_text: str | None) -> str | None:
    ...
    for line in lines[:40]:
        ...
        if heading:
            ...
            if _is_probable_title_line(candidate, is_heading=True):
