  ...
  return useMutation({
    ...
    onSuccess: (result: SourceResponse, variables) => {
      ...
      if (variables.notebooks) {
        ...
        queryClient.invalidateQueries({
          queryKey: QUERY_KEYS.summaries(variables.notebook_id),
          refetchType: 'active'
        })
      }
...
export function useFileUpload() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: QUERY_KEYS.summaries(variables.notebookId)
      })
      toast({
        title: t.common.success,
...
export function useScanNotebookDuplicateSources() {
  ...
  return useMutation<DuplicateSourceGroupResponse[], unknown, string>({
    mutationFn: async (notebookId: string) => {
      const { notebooksApi } = await import('@/lib/api/notebooks')
      return notebooksApi.listDuplicateSources(notebookId)
    },
    onError: (error: unknown) => {
      toast({
        title: t.common.error,
        description: getApiErrorMessage(error, (key) => t(key), 'Failed to scan duplicate sources'),
        variant: 'destructive',
      })
    },
  })
...
export function useCleanupNotebookDuplicateSources() {
  ...
  return useMutation<DuplicateCleanupResponse, unknown, string>({
    mutationFn: async (notebookId: string) => {
      const { notebooksApi } = await import('@/lib/api/notebooks')
      return notebooksApi.cleanupDuplicateSources(notebookId)
    },
    onSuccess: (result, notebookId) => {
      ...
      toast({
        title: t.common.success,
        description: `Removed ${result.removed_count} duplicates and unlinked ${result.unlinked_count} shared sources.`,
      })
    },
    onError: (error: unknown) => {
      toast({
        title: t.common.error,
        description: getApiErrorMessage(error, (key) => t(key), 'Failed to clean duplicate sources'),
        variant: 'destructive',
      })
    },
  })
