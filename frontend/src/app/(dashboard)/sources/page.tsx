...
import { FileText, Link as LinkIcon, Upload, AlignLeft, Trash2, ArrowUpDown, Database, Loader2 } from 'lucide-react'
...
import { useZoteroImport } from '@/lib/hooks/use-zotero-import'
import { ZoteroImportPanel } from '@/components/sources/ZoteroImportPanel'
...
export default function SourcesPage() {
  ...
  const refreshSourcesAfterImport = useCallback(() => fetchSources(true), [fetchSources])
  const zotero = useZoteroImport(refreshSourcesAfterImport)
    ...
    <AppShell>
      <div className="flex flex-col h-full w-full max-w-none px-6 py-6">
        <div className="mb-6 flex-shrink-0 flex items-start justify-between gap-4">
          ...
          <Button onClick={() => zotero.setShowZoteroPanel((v) => !v)} disabled={zotero.importing || zotero.loadingCollections}>
            {zotero.importing || zotero.loadingCollections ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Database className="h-4 w-4 mr-2" />}
            Import from Zotero
          </Button>
        </div>

        {zotero.showZoteroPanel && (
          <ZoteroImportPanel zotero={zotero} language={language} />
        )}
